...
use App\EmailTemplate as Model;
...
class EmailTemplateMailer
{
    ...
    public function getAttributes($slug)
    {
        if (empty(static::$cached[$slug]) === true) {
            static::$cached[$slug] = EmailTemplate::where('handle', '=', $slug)
                ->first();
        }
    ...
    public function send($slug, $data, $closure)
    {
        ...
        if (empty($attributes->from_address) === false) {
            $this->mailer->alwaysFrom($attributes->from_email, $attributes->from_name);
        }

        $sended = $this->mailer->send($view, $data, $closure);
