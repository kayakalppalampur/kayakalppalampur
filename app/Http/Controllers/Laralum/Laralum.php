...
class Laralum extends Controller
{
    ...
    public static function permissions($type = null, $data = null)
    {
        ...
        //return Permission::whereBetween('id',[1,16])->get();
    ...
    public static function settings()
    {
        ...
        if (\Auth::check()) {
            if (\Auth::user()->getDepartmentColor()) {
                $setting->header_color = \Auth::user()->getDepartmentColor();
                $setting->button_color = \Auth::user()->getDepartmentColor();
            }
        }
        return $setting;
    }
    ...
    public static function permissionToAccess($slug)
    {
        ...
        foreach ($slug as $sl) {
            if (Laralum::loggedInUser()->hasPermission($sl)) {
                $ok = true;
            }
        }
        if ($ok == false) {
    ...
    {
        if (!Laralum::isAdmin($user) && !Laralum::isReception($user) && !Laralum::isAccount($user) && !Laralum::isDoctor($user) && !Laralum::isInventory($user) && !Laralum::isKitchen($user) && !Laralum::isPatient($user)  && !Laralum::isLabAttendant($user)) {
    ...
    public static function isDoctor($user)
    {
    ...
    public static function isAccount($user)
    {
        return $user->isAccount();
    }

    public static function isReception($user)
    {
        return $user->isReception();
    }

    public static function isLabAttendant($user)
    {
        return $user->isLabAttendant();
    }
    ...
    {
        return DiscountOffer::getTypeOptions();
    }

    public static function isInventory($user)
    {
        return $user->isInventory();
    }

    public static function isKitchen($user)
    {
        return $user->isKitchen();
    }
    ...
    public static function getShowField($field, $model = null)
    {
        if ($model != null) {
            if (method_exists($model, 'showField')) {
                return $model->showfield($field);
            }
        }
