                        ...
                        <div class="segment form_spacing_inn">
                            <div class="about_sec signup_bg">
                                <div class="page_title">
                                    ...
                                    <div class="hl_desc">Mention cheif complaints / health issues that you want to
                                        treatment for
                                    </div>
                                </div>
                                @if (count($errors) > 0)
                                ...
                                @endif
                                {!! Form::open(array('route' => [ 'Laralum::user.booking.healthIssues.store', 'booking_id' => $booking->id], 'id' =>  'bookingProcessForm','files'=>true,'method'=>'post')) !!}
                                {{ csrf_field() }}
                                <div class="form-group">
                                    <label>Describe your Health Issues (Chief Complaints) here:</label>
                                    <textarea id="health_issues" style="height: auto!important;" rows="5"
                                              class="user_namer form-control complaints required"
                                              name="health_issues" type="text"
                                              placeholder="Mention the issue(s) in simple words."
                                              autofocus>{{ old('UserProfile.health_issues', $profile->health_issues) }}</textarea>
                                </div>
                                <p class="note">Note: It is recommended to mention your issues now so that doctor
                                    can diagnose you quickly when you arrive.</p>
