...
@section('breadcrumb')
    <div class="ui breadcrumb">
        <a class="section" href="{{ route('Laralum::user.bookings') }}">{{ trans('laralum.booking_list') }}</a>
                        ...
                        <div id="edit_details">
                            ...
                            <div class="panel-group" id="tabs">
                                <div class="panel-default">
                                    ...
                                    {!! Form::open(array('route' => ['Laralum::user.booking.accommodation_request', 'booking_id' => $booking->id] , 'id' => 'bookingProcessForm','files'=>true,'method'=>'post')) !!}
                                                ...
                                                <div class="bookling_coll about_sec">
                                                    <div class="white_bg signup_bg">
                                                        <div class="form-group pull-left field_1_sec">
                                                            ...
                                                            <?php $patient = \Illuminate\Support\Facades\Auth::user()->isPatient();
                                                            ?>
                                                        ...
                                                        <div class="form-group pull-left field_1_sec custom_check">
                                                            <input type="checkbox" value="1" id="cd_service_check_1" name="is_child" @if( $booking->is_child == 1) checked @endif> Child <input id="cd_service_number_1" type="number"  class="input_field" name="child_count" min="1" value="{{ $booking->child_count == 0 ? '' : $booking->child_count }}">
                                                        </div>
                                                        <div class="form-group pull-left field_1_sec custom_check">
                                                            <input id="cd_service_check_2" type="checkbox" value="1" name="is_driver" @if($booking->is_driver == 1) checked @endif> Driver <input id="cd_service_number_2" class="input_field" type="number" name="driver_count" min="1" value="{{ $booking->driver_count == 0 ? '' : $booking->driver_count }}">
                                                        </div>
                                                                ...
                                                                @foreach($members as $member)
                                                                    <div id="old_members_{{ $member->id }}">
                                                                        <div class="form-group pull-left field_1_sec">
                                                                            ...
                                                                            <?php $patient = \Illuminate\Support\Facades\Auth::user()->isPatient();
                                                                            ?>
                                                                            {!! Form::select('member[member_building_id][]',["" => "Select Building"] + App\Building::getBuildingOptions(), old('building_id', $member->building_id),['class'=>'form-control required', 'id' => 'member_building_id', 'required' => 'rerquired',($patient)?'disabled':''])  !!}
                                                                        </div>

                                                                        <div class="form-group pull-left field_1_sec">
                                                                            <label>Floor:</label>
                                                                            {!! Form::select('member[member_floor_number][]',["" => "Select Floor"] + App\Building::getFloorOptions(), old('floor_number', $member->floor_number),['class'=>'form-control required select_floor_number', 'id' => 'member_floor_number', 'required' => 'required',($patient)?'disabled':''])  !!}
                                                                        </div>

                                                                        <div class="form-group pull-left field_1_sec">
                                                                            <label>Booking Type:</label>
                                                                            {!! Form::select('member[member_booking_type][]',["" => "Select Booking Type"] + App\Booking::getBookingTypeOptions(), old('booking_type', $member->booking_type),['class'=>'form-control required', 'id' => 'member_booking_type', 'required' => 'required',($patient)?'disabled':''])  !!}
                                                                        </div>
                                                                        <div class="form-group pull-left field_1_sec">
                                                                            {!! Form::label('member_check_in_date', 'Check in date') !!}
                                                                            {!! Form::text('member[member_check_in_date][]', date('d-m-Y', strtotime(old('check_in_date', $member->check_in_date))),['required', 'class' => 'form-control datepicker', 'id' => 'member_check_in_date',($patient)?'disabled':'']) !!}
                                                                        </div>
                                                                        <div class="form-group pull-left field_1_sec">
                                                                            {!! Form::label('member_check_out_date', 'Check out date') !!}
                                                                            {!! Form::text('member[member_check_out_date][]', date('d-m-Y', strtotime(old('check_out_date', $member->check_out_date))),['required', 'class' => 'form-control datepicker', 'id' => 'member_check_out_date',($patient)?'disabled':'']) !!}
                                                                        </div>
                                                                        <div class="form-group pull-left custom_field patient_name1">
                                                                            <input type="hidden" name="member[id][]" value="{{ $member->id }}"/>
                                                                            <label>Name:</label>
                                                                            {!! Form::text('member[name][]',$member->name,['class' => 'form-control', 'id' => 'name', 'style' => '',($patient)?'disabled':'']) !!}
                                                                        </div>

                                                                        <div class="form-group pull-left custom_field patient_gender">
                                                                            <label>Gender:</label>
                                                                            {!! Form::select('member[gender][]',App\UserProfile::getGenderOptions(), $member->gender,['class'=>'form-control', 'id' => 'gender',($patient)?'disabled':''])  !!}
                                                                        </div>
                                                                        <div class="form-group pull-left custom_field patient_age">
                                                                            {!! Form::label('member[age][]', 'Age') !!}
                                                                            {!! Form::text('member[age][]',$member->age,['class' => 'form-control', 'id' => 'age', 'style' => '',($patient)?'disabled':'']) !!}
                                                                        </div>
                                                                        <div class="form-group pull-left custom_field patient_if_proof">
                                                                            {!! Form::label('member[id_proof][]', 'Id Proof') !!}
                                                                            {!! Form::file('member[id_proof][]',['id' => 'id_proof',($patient)?'disabled':'']) !!}
                                                                            @if($member->id_proof != null) <a href="{{  \App\Settings::getDownloadUrl($member->id_proof)}}">Download</a> @endif
                                                                        </div>

                                                                        <div class="form-group pull-left field_1_sec custom_check">
                                                                            <input id="c_service_check_{{ $x }}" type="checkbox" value="1" name="member[member_is_child][]" @if( $member->is_child == 1) checked @endif> Child <input type="number" class="input_field" id="c_service_value_{{ $x }}" name="member[member_child_count][]" min="1"  value="{{ $member->child_count == 0 ? '' : $member->child_count}}">
                                                                        </div>
                                                                        <div class="form-group pull-left field_1_sec custom_check">
                                                                            <input id="d_service_check_{{ $x }}" type="checkbox" value="1" name="member[member_is_driver][]" @if( $member->is_driver == 1) checked  @endif> Driver <input id="d_service_value_{{ $x }}"  class="input_field" type="number" name="member[member_driver_count][]" min="1"  value="{{ $member->driver_count == 0 ? '' : $member->driver_count }}">
                                                                        </div>
                                                        ...
                                                        <div class="form-group btn_signup_con span10">
                                                            <button type="submit" id="submit"
                                                                    class="ui blue submit button save_btn_signup" {{($patient)?'disabled':''}}> Save
                                                            </button>
                                                        </div>
                                                    </div>
                                                </div>
...
// And increment that number by 1
                        ...
                        $("#clone"+num).find('#member_check_in_date').datepicker({dateFormat: "dd-mm-yy", autoclose: true, minDate: 0})
                        $("#clone"+num).find('#member_check_out_date').datepicker({dateFormat: "dd-mm-yy", autoclose: true, minDate: 0})
