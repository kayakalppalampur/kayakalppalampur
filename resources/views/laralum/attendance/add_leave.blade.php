...
<div class="ui one column doubling stackable">
    <div class="ui very padded segment">
        <div class="column about_sec hospital_info role_edt">
            <form id="attendance_form" class="form form form_cond_lft" method="POST" action="{{ route('Laralum::attendance.add_leave', ['user_id' => $user->id ]) }}">
                {{ csrf_field() }}
                    <input type="hidden" name="created_by" value="{{ \Auth::id() }}">
                    <div class="field">
                        <select name="user_id" class="form-control">
                            @foreach(\App\Staff::all() as $all_user)
                                <option value="{{ $all_user->id }}" {{ $user->id == $all_user->id ? 'selected' : "" }}>{{ $all_user->name }}</option>
                ...
                {{--<input type="hidden" name="user_id" value="{{ $user->id }}">--}}
                    <div class="field">
                        <textarea type="text" name="comment" id="comment" class="form-control" placeHolder="Add Comment"></textarea>
                    </div>
                    <div class="field">
                        <input type="text" name="dates" id="dom-id" class="form-control" value="{{ $date }}" required>
                    </div>
