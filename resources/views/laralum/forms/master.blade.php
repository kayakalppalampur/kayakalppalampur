...
@foreach($fields as $field)
   <?php
       ...
       if($empty_value or !isset($row)) {
           ...
           if (isset($row)) {
               $show_field = \App\Http\Controllers\Laralum\Laralum::getShowField($field, $row);
               $show_field = str_replace('_', ' ', ucfirst($show_field));

           }else{
                $show_field = \App\Http\Controllers\Laralum\Laralum::getShowField($field);
                $show_field = str_replace('_', ' ', ucfirst($show_field));
           }
   ...
   @if($editor_script or $code_script)
      ...
      <div class="field">
          ...
          @if($editor_script)
              <textarea class="form-control"  name="{{ $field }}" class="ckeditor">{{ $value }}</textarea>
          @elseif($code_script)
              <pre class="code" id="{{ $field }}-code">{{ $value }}</pre>
              <textarea class="form-control" hidden name="{{ $field }}" id="{{ $field }}">{{ $value }}</textarea>
              <script>
   ...
   @elseif($relation)
        ...
        <div class="field">
            <label>{{ $show_field }}</label>
            <select name="{{ $field }}" id="{{ $field }}" class="ui search dropdown form-control">
            @if($field == 'department_id') <option <?php if($value == 0){ echo "selected"; } ?> value="0">All</option> @endif
       ...
       @if($type == 'string')
                        ...
                        <div class="ui fluid search selection dropdown" id="{{ $field }}_dropdown">
                            ...
                            <div class="menu">
                                ...
                                <?php $cc_field_value = array_search($country, $countries); ?>
                                    <div class="item no-disable" data-value="{{ $cc_field_value }}">@if(in_array($cc_field_value, $no_flags))<i class="help icon"></i>@else<i class="{{ strtolower($cc_field_value) }} flag"></i>@endif{{ $country }}</div>
                                @endforeach
                            </div>
                        </div>
                    </div>
               @else
                   ...
                   <div class="field @if($error) error @endif">
                        <label>{{ $show_field }}</label>
                        <input class="form-control" type="{{ $input_type }}"  id="{{ $field }}" name="{{ $field }}" placeholder="{{ $show_field }}" value="{{ $value }}">
                        @if($error)
           ...
           @if($field == $confirm)
               @if($type == 'string')
                   ...
                   <div class="field @if($error) error @endif">
                        ...
                        <input class="form-control" type="{{ $input_type }}"  id="{{ $field }}_confirmation" name="{{ $field }}_confirmation" placeholder="{{ $show_field }} confirmation" value="{{ $value }}">
                        @if($error)
                            <div class="ui pointing red basic label">
                                {{ $error }}
                            </div>
               ...
               @elseif($type == 'boolean')
                   ...
                   <div class="inline field">
                       <div class="ui slider checkbox">
                           <input <?php if($value){ echo "checked='checked'"; } ?> type="checkbox" tabindex="0" class="hidden form-control" name="{{ $field }}_confirmation">
                           <label>{{ $show_field }} {{ trans('laralum.confirmation') }}</label>
                       </div>
                       @if($error)
                           <div class="ui left pointing red basic label">
