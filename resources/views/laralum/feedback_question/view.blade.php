                ...
                <table class="ui table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @if(\Auth::user()->isAdmin())
                                <td>{{ isset($feedback_question->user->name ) ? $feedback_question->user->name  : ""}}</td>
                            @endif
                        </tr>
                        <tr>
                            <th>Description</th>
                            ...
                            <tr>
                                <td>{!! $feedback_question->description !!}</td> </tr>
                        ...
                        <div class="table-responsive">
                            <div class="comment_main_list">
                                <div class="comment_listing">
                                    <ul>
                                        @foreach($feedback_question->replies as $reply)
                                            <li>
                                                <div class="header">
                                                    {{ $reply->user->name }}
                                                </div>
                                   ...
                                   @if($feedback_question->status != \App\DiscountOffer::STATUS_RESOLVED)
                                        @if(\Auth::user()->isAdmin() || $feedback_question->created_by == \Auth::user()->id)
                                            <div class="comment_wrapper">
                                                <form  method="post" id="reply_form" action="{{ route('Laralum::feedback_question.send_reply', ['feedback_question_id' => $feedback_question->id]) }}">
                                                    {!! csrf_field() !!}
