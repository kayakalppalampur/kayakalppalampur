...
@section('content')
    ...
    @php
        if($booking->patient_type == \App\Booking::PATIENT_TYPE_IPD)
           $route = route('Laralum::ipd.bookings.account', ['booking_id' => $booking->id]);
        else
           $route = url('/admin/booking/account/'.$booking->id)
    @endphp


    <div class="detail_container" id="mySelector" style="width:1000px;max-width:1000px;" >
        <div class="receipt-for">
            <p>@if(!isset($print)) <a id="print" class="btn btn-primary ui button blue">Print</a> <a id="back" href="{{ $route }}">Back</a>  @endif
            </p>
        </div>
