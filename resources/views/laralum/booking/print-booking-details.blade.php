...
*/
    ...
    <div class="token-receipt" id="mySelector" style="width:1000px;max-width:1000px;" >
        ...
        <div class="receipt-for">
            <p>@if(!isset($print)) <a id="print" class="btn btn-primary ui button blue">Print</a> <a id="back" class="btn btn-primary ui button blue" href="{{ isset($back_url) ? $back_url : url('/admin/booking/'.$booking->id.'/details') }}">Back</a>  @endif
            </p>
        </div>
        <div class="token-detail-box" id="">
            ...
            <dl>
                ...
                <div class="dt-dd detail_inline">
                    <dt>Created at:</dt>
                    <dd>{{ date("d-m-Y", strtotime($booking->getDate('check_in_date'))) }}</dd>
                    <dt>Status:</dt>
        ...
        <div class="token-detail-box" id="">
            ...
            <dl>
                <div class="dt-dd detail_inline">
                    <dt>Address:</dt>
                    <dd>{!! $booking->getAddress('address1') ? $booking->getAddress('address1').', '.$booking->getAddress('address2').', '.$booking->getAddress('city').', '.$booking->getAddress('zip').'<br>'.$booking->getAddress('state').'<br>'.$booking->getAddress('country') : ""!!}</dd>
                </div>
                <div class="dt-dd detail_inline">
                                ...
                                @foreach($booking->bookingRoomsAll as $d)
                                    <tr>
                                        ...
                                        <td>{{ date("d-m-Y", strtotime($d->check_in_date)) }}
                                            to {{ $discharge == true ?  date("d-m-Y") : date("d-m-Y", strtotime($d->check_out_date)) }} {{--> date("d-m-Y") ? "Till date" : date("d-m-Y", strtotime($d->check_out_date)) }}--}}</td>
    ...
    <div class="detail_container">
        ...
        @endif
                    ...
                    <table class="ui table table_cus_v bs">
                        ...
                        <tr>
                            ...
                            @if($genitorinary->LMP != null || $genitorinary->LMP_doctor != null)
                                <th>LMP</th>
                                <td>{{ date("d-m-Y", strtotime($genitorinary->LMP)) }}</td>
            ...
            @if($ashtvidh->id != null)
                <div class="token-detail-box table_head_lft">
                            ...
                            @foreach($booking->treatments as $treatment)
                                @foreach($treatment->treatments as $pat_treatment)
                                    @if($pat_treatment->status == \App\PatientTreatment::STATUS_COMPLETED)
                                        <p>
                                            <span style="margin:10px ;">Treatment: {{ $pat_treatment->treatment->title }}</span><span> Date: {{ date("d-m-Y", strtotime($treatment->treatment_date)) }}</span><span> Price: {{ $pat_treatment->treatment->price }}</span>
                                        </p>
                                    @endif
                                @endforeach
                            @endforeach
                        ...
                        <table>
                            ...
                            @foreach($booking->labTests as $lab_test)
                                <tr>
                                    ...
                                    <td>{{ date("d-m-Y", strtotime($lab_test->date)) }}</td>
