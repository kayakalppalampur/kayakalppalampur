...
@section('content')
    ...
    <div class="token-receipt" id="mySelector" style="width:1000px;max-width:1000px;">
        <div class="receipt-for">
            <p>@if(!isset($print)) <a id="print" class="btn btn-primary ui button blue">Print</a> <a id="back" class="btn btn-primary ui button blue" href="{{ isset($back_url) ? $back_url : url('/admin/booking/generate-token/'.$token->booking_id) }}">Back</a>  @endif</p>
        </div>
      <div class="token-detail-box" id="">
        ...
        <p class="visit-dr">TOKEN TO VISIT DOCTOR</p>
        <dl>
            <div class="dt-dd"> <dt>UHID:</dt>
                <dd>{{ $token->booking->getProfile('uhid') }}</dd></div>
 <div class="dt-dd"> <dt>KID:</dt>
  <dd>{{ $token->booking->getProfile('kid') }}</dd></div>
  <div class="dt-dd"> <dt>Date:</dt>
  <dd>{{  date('d M, Y h:i a', strtotime($token->created_at->setTimezone(env('TIMEZONE')))) }}</dd></div>
  <div class="dt-dd"> <dt>Full Name:</dt>
  <dd>{{ $token->booking->getProfile('first_name').' '.$token->booking->getProfile('last_name') }}</dd></div>
 <div class="dt-dd">  <dt>Age:</dt>
  <dd>{{ $token->booking->getProfile('age') }}</dd></div>
    <div class="dt-dd"> <dt>Sex:</dt>
  <dd>{{ \App\UserProfile::getGenderOptions($token->booking->getProfile('gender')) }}</dd></div>
</dl>
<p class="to-no">Token Number</p>
<div class="valid-till">
	<div class="day"><h2>{{ $token->token_no }}</h2></div>
  <div class="date" style="     background-color: #ccc;
            float: left;
            padding: 8px;
            width: 75%;">  <h3>Valid only for<br />
{{ date('d M, Y', strtotime($token->end_date)) }}</h3></div>
</div>
<h3 class="dr-name">{{ $token->department->title }} {{ @$token->doctor->name }}</h3>
<p>Thank you!</p>
</div>
