            ...
            <div class="ui very padded segment">
                ...
                <table class="ui table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $kitchen_item->title }}</td>
                            @if(\Auth::user()->isAdmin())
                                <td>{{ isset($kitchen_item->user->name ) ? $kitchen_item->user->name  : ""}}</td>
                            @endif
                        </tr>
                        <tr>
                            <th>Description</th>
                            ...
                            <tr>
                                <td>{!! $kitchen_item->description !!}</td> </tr>
                        ...
                        <div class="table-responsive">
                            <div class="comment_main_list">
                                <div class="comment_listing">
                                    <ul>
                                        @foreach($kitchen_item->replies as $reply)
                                            <li>
                                                <div class="header">
                                                    {{ $reply->user->name }}
                                                </div>
                                   ...
                                   @if($kitchen_item->status != \App\DiscountOffer::STATUS_RESOLVED)
                                        @if(\Auth::user()->isAdmin() || $kitchen_item->created_by == \Auth::user()->id)
                                            <div class="comment_wrapper">
                                                <form  method="post" id="reply_form" action="{{ route('Laralum::kitchen_item.send_reply', ['kitchen_item_id' => $kitchen_item->id]) }}">
                                                    {!! csrf_field() !!}
